...
const commonFields = {
  ...
  openTime: z.preprocess(
    nullToUndefined,
    z.string().regex(timeRegex, 'Enter a valid open time (HH:MM).').optional(),
  ),
  closeTime: z.preprocess(
    nullToUndefined,
    z.string().regex(timeRegex, 'Enter a valid close time (HH:MM).').optional(),
  ),
...
export const facilityFormSchema = z
  .object({
    ...
    vehicleTypes: vehicleTypesField.min(1, 'Select at least one vehicle type.'),
  })
