...
import { revokeInviteAction } from '@/lib/invite-actions'
import type { InviteActionResult } from '@/lib/invite-actions'

interface Props {
  id: string
}

const INITIAL_STATE: InviteActionResult = { ok: true }
...
export function RevokeInviteButton({ id }: Props) {
  const t = useTranslations('onboarding')
  const [open, setOpen] = useState(false)
  const [state, formAction] = useActionState(revokeInviteAction, INITIAL_STATE)
