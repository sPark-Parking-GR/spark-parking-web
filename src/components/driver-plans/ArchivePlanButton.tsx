...
import { archiveDriverPlanAction } from '@/lib/driver-plan-actions'
...
export function ArchivePlanButton({ planId, planName, subscribers, iconOnly = false }: Props) {
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const result = await archiveDriverPlanAction(planId, trimmedReason || undefined)
      if (!result.ok) {
  ...
  return (
    <>
      <button
        type="button"
        className={iconOnly ? 'btn btn--icon btn--ghost' : 'btn btn--sm btn--secondary'}
      ...
      >
        ...
        {iconOnly ? null : t('archive.trigger')}
      </button>

      <Modal open={open} onClose={() => setOpen(false)} title={t('archive.title')}>
        <p className="modal__text">
          {t('archive.body', { name: planName })}
